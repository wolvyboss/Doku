...
Usage :
  ...
  python generate.py --days 30 --from 2026-06-01
...
def load_theme_data(theme_id):
    entries    = load(f'data/{theme_id}/entries.json')
    categories = load(f'data/{theme_id}/categories.json')
    return entries, categories

# ─────────────────────────────────────────────────────────────
# ENRICHISSEMENT VILLES (attrs lettres auto)
# ─────────────────────────────────────────────────────────────
...
def generate_theme(theme, entries, categories, start_date, nb_days):
    ...
    seed_offset      = theme.get("seed_offset", 0)
    familles_strictes = theme.get("familles_strictes", [])

    # Enrichissement auto pour les thèmes villes
    if "villes" in theme_id:
    ...
    for i in range(nb_days):
        ...
        result = generate_grille(seed, compat, cat_ids, categories,
...
if __name__ == "__main__":
    ...
    # Créer le dossier grilles/ si besoin
    ...
    for theme in themes:
        if not theme.get("active", True):
            print(f"\n⏭️  {theme['label']} ignoré (inactif)")
            continue
        entries, categories = load_theme_data(theme["id"])
